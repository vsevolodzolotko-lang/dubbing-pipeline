...
**Output**: archives the *previous* lesson into its own `05_archive/{lesson_id}_{date}/` folder (lesson-scoped, by exact `file_id` — see the archive chain below), copies the new input into its own archive folder, then triggers W1 → W2 → W3 in sequence on the new file. W3 (decoupled, fire-and-forget) posts the completion Slack notification itself when the last language finishes.
...
| Once Per Run (Code) | Collapses N Parse Filename items into a single sentinel item and emits `archive_run_at` (the run token stamped by the token branch). Fans out to both the archive chain and `Read Config (Start)`. |
| Read Config (Archive) | Pulls all `drive_*_folder_id` keys + `drive_archive_folder_id` + `last_archive_folder_id`. Separate from the post-W3 `Read Config` because the archive chain runs at the head of the workflow. |
| **Archive chain (lesson-scoped, ~20 nodes)** | Self-archive model: each lesson owns one `05_archive/{lesson_id}_{ts}/` folder, filled across two run boundaries. All Drive + Sheets calls go through HTTP Request nodes with `predefinedCredentialType` (`googleDriveOAuth2Api` / `googleSheetsOAuth2Api`) — `helpers.httpRequestWithAuthentication` is blocked in some n8n Code Node sandboxes. **Archive the previous lesson** (runs at this run's start, for the lesson that just finished): `Read Localizations (Archive)` → `Plan Prev Archive` (Code: prev `lesson_id` from `segment_id` prefix; exact `02_output` file ids from `localizations.audio_drive_file_id` — **no folder list, no pagination**; skips if `last_archive_folder_id` empty / sheet empty) → `Has Prev To Archive?` (IF) → `Plan Output Lists` → `List Output Files` (HTTP GET, full/vtt by `name contains '{lesson}_full_'`, ≤7 each) → `Plan Archive` (Code: routes files by extension, computes `_INCOMPLETE` if a full WAV is missing for any active lang) → `Snapshot Sheet (Prev)` (HTTP copy into the prev lesson's folder; `onError=stopWorkflow` BEFORE any move — no data loss) → `Plan Subfolders` → `Create Subfolder` (02/03/04 under the existing root) → `Plan Moves` → `Move File` (HTTP PATCH `addParents/removeParents`, retry 3, `continueRegularOutput`) → `Clear Sheet Tabs` (HTTP `values:batchClear` on `segments`/`localizations`; `voices`/`prompts`/`config` untouched). **Prepare this lesson** (convergence of both IF branches): `Plan Lesson N` (Code: validates `drive_archive_folder_id`, builds `{lesson_id}_{ts}` name) → `Create Lesson N Root` → `Create Lesson N Input Subfolder` (`01_input`) → `Plan Input Copy` → `Copy Input to Archive` (HTTP copy the input mp3 in; `onError=stopWorkflow`) → `Store Archive Folder Id` (Sheets: `last_archive_folder_id` = new root, for the next run) → `Pass Lessons (after Prepare)` (re-emits Parse Filename items) → `Execute W1`. **Secure-source delete** (after W1): `Execute W1 (STT)` → `Plan Input Delete` → `Delete Input File` (HTTP DELETE by exact `file_id` — W1 has downloaded the audio; batch-safe) → `Pass Lessons (after W1)`. |
...
3. Archive chain → check that each HTTP Request node (`List Output Files`, `Snapshot Sheet (Prev)`, `Create Subfolder`, `Move File`, `Clear Sheet Tabs`, `Create Lesson N Root`, `Create Lesson N Input Subfolder`, `Copy Input to Archive`, `Delete Input File`) has its `predefinedCredentialType` credential bound correctly (Drive ones use `googleDriveOAuth2Api`; `Clear Sheet Tabs` uses `googleSheetsOAuth2Api`). The Sheets nodes `Read Localizations (Archive)` and `Store Archive Folder Id` use the `googleSheets` node credential. After import n8n usually keeps the binding, but verify on first opening.
