...
| expansion_attempts | number | How many times expansion fired for this segment × lang. Pre-2026-05-25 this came from W3 Check Timing + Pad inline loop (max 2). After Phase 2 architecture: `1` = Phase 2 batch (Expand → Verify → Editor) accepted a new translation and re-TTS produced an in-budget WAV; `0` = either no Phase 2 candidate (already long enough) or Phase 2 candidate but rejected (overshoot, no-change, or LLM failure). |
