...
| fade_in_sec | number | (Added 2026-06-23, staged audio-review UI) Linear fade-in applied to the **start** of this language's dub clip, in seconds. Set by dragging the block's top-left grip on the audio timeline (Dubbing Studio). `0` = no fade. Clamped so `fade_in_sec + fade_out_sec ≤` the per-language slot length. **Etap M: mock-only** (stored in the mock store; the render stage would bake the envelope into the per-lang full file). Etap P: the assemble/render node applies the ramp; column to be added to the live `localizations` tab. |
| fade_out_sec | number | (Added 2026-06-23, staged audio-review UI) Linear fade-out applied to the **end** of this language's dub clip, in seconds. Set by dragging the block's top-right grip on the audio timeline. See `fade_in_sec` for clamping/staging notes. **Superseded for cut clips by per-clip fades inside `clips`** (this column stays the fade of an uncut/default clip). |
| clips | JSON text | (Added 2026-06-23, staged audio-review UI) The per-language dub as a list of **independent audio clips** the operator cut/moved/trimmed/faded/deleted on the audio timeline. JSON array of `{ id, start, end, srcStart, srcEnd, sourceDur, fadeIn, fadeOut }` (seconds; `src*` = which slice of the source audio a piece plays, 1:1). Empty/absent ⇒ one default clip spanning the slot with `fade_in_sec`/`fade_out_sec`. **Etap M: mock-only** (stored in the mock store). Etap P: the render node applies this edit list when assembling the per-lang file; column to be added to the live `localizations` tab. |
...
| `tuning_advisor_system` | — | (Optional, added 2026-06-23) System prompt for the operator UI **Tuning** tab LLM advisor (`ui/server/services/qualityAdvisor.js`) — turns a run's quality metrics into config/voice/prompt recommendations. Built-in default in the UI server; add this row only to tune the advisor itself. Not read by the n8n pipeline. |
