...
| `localization_run_completed_token` | *(runtime — managed by workflows, do not edit)* | W3 Synthesize Mark Run Completed (write), W_Master Check Single-Flight (read) | Single-flight release marker. W3 writes it `= localization_run_token` on the **final** language (`is_last_lang`), signalling the whole run is done. W_Master's start-up guard treats a run as still active only while `run_token` is set, not aborted, **and** `completed_token !== run_token`. Lets a normally-finished run release the lock immediately (no time window). **Operators never touch this row.** |
| `localization_stale_run_minutes` | `60` (default if unset) | W_Master Check Single-Flight (read) | Freshness escape for the single-flight guard: if the active `run_token` is older than this many minutes, the guard assumes the run crashed (it never wrote `completed_token`) and lets a new run start — so a dead run can't wedge the pipeline forever. Raise it only if a legitimate run can exceed 60 min. |
