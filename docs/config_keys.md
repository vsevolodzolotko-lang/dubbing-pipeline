...
W3 Phase 1 TTS concurrency is **not** a config-sheet key — it's the `batchSize` on the **`Loop Over Items`** node (default **7**). As of 2026-05-27 the initial ElevenLabs call runs **inside the `Check Timing + Pad` Code node** (the separate `ElevenLabs TTS` HTTP node was removed); that node synthesizes its whole input batch in parallel via `Promise.all`, so peak simultaneous ElevenLabs calls = `Loop Over Items` batchSize (retries within a job stay sequential). Raise `batchSize` (e.g. 14) in the n8n UI for more parallelism on a high ElevenLabs tier; lower it toward 1 to throttle. The `Rate Limit Guard` wait between batches is now **0.2s** (was 1.5s per item).
...
| `max_speed_up_delta` | `0.15` | W3 Check Timing + Pad, W3 Phase 2: Batch LLM+TTS | Max speed-UP above this voice's configured `speed` (voices tab) for the shorten path. Cap = `voice.speed + delta`; steps `[voice.speed + delta·⅔, voice.speed + delta]`. For a 1.0 voice → ceiling 1.15 (unchanged); for a 0.8 voice → ceiling 0.95 (was jarringly forced to absolute 1.15). Replaces the old absolute `max_speed`. **As of 2026-05-28**: Phase 2 re-TTS also uses this. When expanded text slightly overshoots speechBudget at voice.speed (typical for clause-preserving expansions in short slots), Phase 2 retries the same TTS at the same two speed-up steps before classifying the cell as `overshoot`. Without this, restored expansions ~5-10% over slot at default speed were rejected and Phase 1 audio (often with the missing clause that triggered Phase 2) was kept. |
