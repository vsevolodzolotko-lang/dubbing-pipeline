...
| `min_segment_piece_duration_sec` | `1.5` | W1 Segment Transcript | Each side of a **duration-driven** (long-sentence) split must be ≥ this many seconds. Prevents creating micro-segments (e.g. a 0.3s "Just." piece) when chopping a long flowing sentence. Does **not** gate hard-pause splits — those use `min_hard_pause_piece_sec`. As of 2026-05-31. |
| `hard_pause_split_sec` | `0` *(disabled)* | W1 Segment Transcript | Split a sentence at any internal word-gap ≥ this many seconds **regardless of total duration**. The duration-based split (`max_segment_duration_sec`) only inspects gaps once a sentence already exceeds the cap, so a single Deepgram sentence under the cap that contains a long internal pause (e.g. "One more time to the left **[4s]** and back to the right." at 8.6s, or "Breathe in **[4.5s]** And breathe out…" at 11.5s) stayed whole. Set ~`1.5`–`2.0` to make audible pauses close a segment and open a new one; each resulting half need only be ≥ `min_hard_pause_piece_sec` (so a brief cue like "Breathe in" ≈ 0.85s before a real silence still gets its own segment). Below ~1.5 risks over-splitting natural breath pauses in somatic lessons. `0` = disabled (default, fully back-compatible). When only a hard pause (not over-length) triggers the split, candidate gaps are restricted to ≥ this value so the cut lands on the real silence. As of 2026-06-16. |
| `min_hard_pause_piece_sec` | `0.4` | W1 Segment Transcript | Piece floor for **hard-pause** splits (separate from `min_segment_piece_duration_sec`). The silence itself justifies the boundary, so a short cue around it ("Breathe in", "Stop.") is a legitimate standalone segment and shouldn't be blocked by the 1.5s long-sentence floor. Only sub-floor degenerate slivers are still rejected. Raise it if hard pauses produce too many stubby segments; lower it (toward `0`) to honour pauses even around single words. Only consulted when `hard_pause_split_sec > 0`. As of 2026-06-17. |
...
| `movement_borrow_max_sec` | `2.0` | W3 Check Timing + Pad | **Optional (default 2.0 in code — no row needed unless overriding).** Per-segment breath-borrow cap for **movement** cues specifically (`movement_keywords` non-empty OR `segment_type == 'movement'`, e.g. Inhale/Hold/Exhale). Movement cues may now extend past `en_duration_sec` into trailing silence like any segment, but bounded by this (tighter or equal to `max_borrow_per_segment_sec`, which still binds via `effective_slot_sec`). Set to `0` to restore the strict `en_duration` lock for hard video-synced lessons. Added 2026-06-05 — strict-lock was hard-truncating one-word cues whose translations are full phrases. See DECISIONS `MOVEMENT_CUES_ALLOW_BOUNDED_BREATH_BORROW`. |
...
| `short_seg_threshold_sec` | *(dead — 2026-06-04)* | — | Previously gated breath-borrow ("only segments shorter than this can borrow trailing silence"). Removed: ALL segments can now borrow trailing silence — non-movement up to `effective_slot_sec`, movement cues up to `movement_borrow_max_sec` (default 2.0). The config key is no longer read by code; safe to delete the row from the config sheet. See [DECISIONS.md](../DECISIONS.md) entries `PERMISSIVE_BORROW_FOR_NONMOVEMENT_SEGMENTS_2026-06-04` and `MOVEMENT_CUES_ALLOW_BOUNDED_BREATH_BORROW_2026-06-05`. |
...
| `slack_signing_secret` | *(required for the Stop button)* | W_Abort Prep Signature / Verify & Parse Action | Slack app **Signing Secret** (Basic Information → App Credentials). W_Abort receives the "🛑 Stop localization" button click as a signed Slack interactivity POST and verifies `HMAC-SHA256(secret, 'v0:{ts}:{raw_body}')` against the `X-Slack-Signature` header (+ 5-minute timestamp replay guard) **before** writing the abort flag. This is what protects the otherwise-public `…/webhook/slack-actions` endpoint — a forged/unsigned POST is rejected and no run is stopped. If missing, every stop request is rejected (button does nothing). Set up: enable **Interactivity & Shortcuts** in the Slack app, set Request URL to W_Abort's `slack-actions` production webhook URL, then copy the Signing Secret here. |
| `localization_run_token` | *(runtime — managed by workflows, do not edit)* | W_Master Write Run Token (write), W3 Dispatch Check Abort (read) | Identifies the currently-running localization. W_Master stamps it at run start (= the run's ISO start time) and the Stop button is bound to this exact value. **Operators never touch this row** — it is overwritten automatically each run. |
| `localization_abort_token` | *(runtime — managed by workflows, do not edit)* | W_Abort Write Abort (write), W3 Dispatch Check Abort (read), W_Master clears at run start | Cooperative kill-switch. The Stop button (via W_Abort) sets this to the run_token of the run whose button was clicked; W_Master clears it (`''`) at the start of every run. A checkpoint halts only when `localization_abort_token === localization_run_token` (both non-empty), so a stale stop from a previous run never blocks a fresh one. **Operators normally never edit this** — but setting it by hand equal to the current `localization_run_token` is a valid emergency manual stop if Slack is unavailable. |
