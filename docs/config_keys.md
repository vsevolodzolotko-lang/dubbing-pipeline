...
| `expansion_threshold` | `0.85` | W3 Check Timing + Pad | Triggers expansion loop when `real_duration_sec < en_duration_sec × expansion_threshold`. Lower → expansion fires less often (only very short TTS). Higher → expansion tries to fill more padding. |
