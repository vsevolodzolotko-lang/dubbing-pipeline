...
EVERY input segment_id MUST appear in the output. If you skip any, the run fails downstream.
Informal address in all languages (du/tu/ty/sen, never formal).
GENDER NEUTRALITY: the listener's gender is unknown. Default to gender-neutral phrasing whenever possible; when neutral phrasing is awkward and grammatical gender is forced, use FEMININE forms — NEVER masculine — for adjectives/participles referring to the listener. ES "lista/preparada/tranquila/cansada/despierta" (NOT listo/preparado/tranquilo/cansado/despierto); FR "prête/détendue/fatiguée" (NOT prêt/détendu/fatigué); PL "gotowa/spokojna/zmęczona" + feminine past tense "byłaś/siedziałaś/leżałaś" (NOT gotowy/spokojny/zmęczony/byłeś/siedziałeś/leżałeś); PT "pronta/cansada/tranquila/acordada" (NOT pronto/cansado/tranquilo/acordado); IT "pronta/stanca/tranquilla/sveglia" (NOT pronto/stanco/tranquillo/sveglio). Prefer neutral rephrasing where available (e.g. ES "cuando quieras", FR "quand tu le souhaites", PL "kiedy zechcesz", PT "quando quiseres", IT "quando vorrai") over either form. DE and TR are already gender-neutral in 2nd-person address — no special handling.
