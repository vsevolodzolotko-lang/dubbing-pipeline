...
After editing any `code_nodes/*.js`, run `npm run sync` (= `node scripts/sync_jscode.js`) to push the change into the workflow JSON. The script is idempotent — re-running on an already-synced state is a no-op. Covers all 19 nodes listed below across W1/W2/W3/W_Regen.
