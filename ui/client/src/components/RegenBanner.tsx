...
export function RegenBanner() {
  ...
  if (regen.active && regen.stale) {
    ...
    return (
      <Bar tone="red">
        <AlertTriangle className="inline-block h-3.5 w-3.5 align-[-0.2em]" strokeWidth={1.75} /> Regen may have failed — {regen.remaining} of {regen.total} rows did not update in ~{mins} min.
        Check n8n executions / Slack (W_Regen has no error workflow — a silent failure is possible).
      </Bar>
    )
  }
  if (regen.active) {
    return <Bar tone="blue"><RefreshCw className="inline-block h-3.5 w-3.5 align-[-0.2em] animate-spin" strokeWidth={1.75} /> Regen in progress — {regen.remaining} of {regen.total} remaining ({regen.elapsedSec ?? 0}s)…</Bar>
  }
  if (doneCount != null) {
    return <Bar tone="green"><RefreshCw className="inline-block h-3.5 w-3.5 align-[-0.2em]" strokeWidth={1.75} /> Regen complete — {doneCount} rows are now REVIEW. Listen and submit a verdict (filter "After regen").</Bar>
  }
  return null
}
