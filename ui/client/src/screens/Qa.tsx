...
import { ArrowRight, Check, Copy, ShoppingBasket } from 'lucide-react'
...
export function Qa() {
  ...
  async function run() {
    ...
    try {
      ...
      if (res.status === 409) { setError('Analysis is already running'); setRunning(false); return }
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-xl font-semibold">AI quality analysis</h1>
        ...
        <button onClick={run} disabled={running}
          ...
          {running ? 'Analyzing…' : report ? 'Analyze again' : 'Run analysis'}
        </button>
        {report?.generatedAt && !running && (
          <span className="text-xs text-gray-400">last: {new Date(report.generatedAt).toLocaleString('uk-UA')}{report.model ? ` · ${report.model}` : ''}</span>
        )}
      </div>
      <p className="mt-1 max-w-prose text-sm text-gray-500 dark:text-gray-400">
        Deep LLM review of translations: formality, gender, false friends / meaning, naturalness.
        Separate from the fast deterministic gate on the Translation page. The review prompt lives in the Prompts tab.
      </p>

      {running && progress && (
        <div className="mt-4 rounded-lg bg-gray-50 px-3 py-2 text-sm dark:bg-[#202023]">
          Languages processed: <b>{progress.done}/{progress.total}</b> · last: <b>{progress.lang}</b> (found {progress.found})
      ...
      {report && !running && (
        report.total === 0 ? (
          <div className="mt-5 rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-900 dark:bg-green-950/50 dark:text-green-300">
            No issues found{report.lessonId ? ` for ${report.lessonId}` : ''}.
          </div>
        ) : (
          <div className="mt-5">
            ...
            <div className="mb-3 space-y-1.5 text-xs">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="w-20 shrink-0 text-gray-400">Importance</span>
                <Chip active={sevFilter === null} onClick={() => setSevFilter(null)}>all {report.total}</Chip>
                {SEV_ORDER.map((s) => (
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="w-20 shrink-0 text-gray-400">Language</span>
                <Chip active={langFilter === null} onClick={() => setLangFilter(null)}>all</Chip>
                {langs.map((l) => (
            ...
            {visible.length === 0 ? (
              <div className="text-sm text-gray-400">No findings for the selected filter.</div>
            ) : (
      ...
      {!report && !running && !error && (
        <div className="mt-6 text-sm text-gray-400">
          Not analyzed yet. Click "Run analysis"{state?.mode === 'mock' ? ' (MOCK mode requires a live gemini_api_key).' : '.'}
        </div>
      )}
    </div>
  )
    ...
    <div className={`appear rounded-md border-l-2 border border-gray-200 ${SEV_BORDER[f.severity]} bg-white p-1.5 text-xs dark:border-[#29292c] dark:bg-[#161617]`}>
      <div className="flex items-center gap-1.5">
        ...
        {f.suggestion && (
          <div className="ml-auto flex gap-1">
            <button onClick={toCart} disabled={!writable} title={writable ? 'add to cart' : 'enable write'}
      ...
      {(f.current || f.suggestion) && (
        <details className="mt-1">
          <summary className="cursor-pointer text-[11px] text-gray-400">text</summary>
          <div className="mt-1 space-y-0.5">
            <Row label="EN" v={f.enText} cls="text-gray-500 dark:text-gray-400" />
            <Row label="now" v={f.current} cls="text-gray-700 dark:text-gray-300" />
