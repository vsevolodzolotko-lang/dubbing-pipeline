...
export function Config() {
  ...
  if (isLoading) return <div className="p-8 text-sm text-gray-400">Loading configuration…</div>
  ...
  for (const row of rows) {
    ...
    if (DEAD_KEYS.has(row.key)) meta = { group: 'Dead keys', label: row.key, type: 'readonly', tooltip: 'Deprecated key — the code does not read it; you can delete the row manually.' }
    if (!meta) meta = { group: 'Other', label: row.key, type: row.masked ? 'secret' : 'readonly' }
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="flex items-center gap-3">
        ...
        {!writable && <span className="rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-700">read-only — {reason}</span>}
      </div>
      <p className="mt-1 max-w-prose text-sm text-gray-500">
        Pipeline settings without opening the sheet. Secrets are masked. Changes affect <b>upcoming</b> lessons.
      </p>

      <div className="mt-5 max-w-3xl space-y-6">
        {orderedGroups.map((g) => {
          if (g === 'Dead keys' && !showDead) {
            return (
              <button key={g} onClick={() => setShowDead(true)} className="text-xs text-gray-400 underline">
                show dead keys ({groups[g].length})
              </button>
            )
          }
          return (
            <section key={g}>
              <h2 className={`mb-2 text-sm font-semibold ${g.startsWith('Секрети') || g.startsWith('Системне') || g === 'Dead keys' ? 'text-gray-500' : 'text-gray-800 dark:text-gray-200'}`}>{g}</h2>
...
function Field({ k, meta, value, masked, writable, reason, onSaved }: {
  ...
  async function save() {
    ...
    try {
      ...
      if (res.ok && d.ok) { setMsg({ kind: 'ok', text: 'Saved' }); onSaved() }
  ...
  async function runCheck() {
    ...
    try {
      ...
      const d = await res.json().catch(() => ({}))
      setCheck(d.ok ? { ok: true, text: 'key works' } : { ok: false, text: String(d.reason || d.status || 'invalid') })
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {editable ? (
          ...
          <div className="flex items-center gap-2">
            ...
            {meta.testable && <button onClick={runCheck} className="rounded border border-gray-300 dark:border-[#3a3a3d] px-2 py-1 text-xs hover:bg-gray-100">Check</button>}
          </div>
        ) : (
